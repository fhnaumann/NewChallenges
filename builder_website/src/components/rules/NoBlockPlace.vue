...
if(!Object.hasOwn(store.rules.enabledRules, 'noBlockPlace')) {
  store.rules.enabledRules.noBlockPlace = structuredClone(toRaw(defaultConfig.rules.enabledRules.noBlockPlace))
}
...
watch(store.rules.enabledRules.noBlockPlace!.exemptions, (newExemptions) => {
...
function updateStore(model: Model = store, newSelectedData: DataRow[]) {
  ...
  model.rules.enabledRules.noBlockPlace!.exemptions = newSelectedData.map((row) => row.code)
}
