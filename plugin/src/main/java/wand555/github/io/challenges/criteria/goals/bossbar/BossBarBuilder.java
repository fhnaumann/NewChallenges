...
public class BossBarBuilder {
    ...
    public float buildProgress() {
        List<BuildProgress> buildProgresses = this.parts.stream().filter(BuildProgress.class::isInstance).map(BuildProgress.class::cast).toList();
        if(buildProgresses.isEmpty()) {
            return 1f;
        }
        if(buildProgresses.size() > 1) {
            JavaPlugin.getPlugin(Challenges.class).getComponentLogger().warn("Encountered a BossBar where more than one part sets a progress value.");
        }
